...
clean: ## reset the branch (drop gitignored files AND the .python-version you created)
	git clean -fdX
...
check: ## verify uv, the .python-version pin, and that uv honours it
	...
	@test -f .python-version || { echo "✘ .python-version missing — create it: echo \"3.13\" > .python-version or uv python pin 3.13"; exit 1; }
